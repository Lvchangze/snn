        ...
        for i in tqdm(range(len(sample_list))):
            ...
            for j in range(self.sent_length):
                if j >= len(text_list):
                    ...
                    # N(0, 1)
                    embedding = (embedding - np.array([mean_value] * self.embedding_dim)) / np.array([np.sqrt(variance_value)] * self.embedding_dim)
                    ...
                    embedding_norm = embedding_norm + np.array([np.abs(left_boundary)] * self.embedding_dim)
...
if __name__ == "__main__":
    tensor_encoder = TensorEncoder(vocab_path="data/glove.6B.100d.txt",
                            data_type="train",
